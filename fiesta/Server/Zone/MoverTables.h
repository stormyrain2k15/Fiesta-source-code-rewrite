...
public:
    ...
    const MoverMainRow* Find(uint8 bClass, uint16 uiLevel) const;
    void                FillRaw(RAWCHARSTAT* pOut, uint8 bClass, uint16 uiLevel) const;
private:
    MoverMainTable() {}
    std::map<uint32, MoverMainRow> m_kRows;       // key = (class<<16)|level
};
...
public:
    ...
    bool                   Load(const std::string& rRoot);
    const MoverAbilityRow* Find(uint8 bClass) const;
private:
    MoverAbilityTable() {}
    std::map<uint8, MoverAbilityRow> m_kRows;
};
