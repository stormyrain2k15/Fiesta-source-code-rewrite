...
void ItemTables::Bind() {
    {
        ...
        ITER_ROWS(t) {
            ...
            rec.uiSetItemIndex  = ShnGetU32(*t, _r, "SetItemIndex");
        ...
        SHINELOG_INFO("ItemInfo.shn: %u rows", (uint32)m_kItems.size());
    }
    {
        ...
        SHINELOG_INFO("ItemInfoServer.shn: %u rows", (uint32)m_kServer.size());
    }
    {
...
void GradeRandomTables::Bind() {
    if (const ShnFile* t = ShnRegistry::Get().GetTable("GradeItemOption")) {
        ITER_ROWS(t) {
            ...
            rec.iValue  = ShnGetI32(*t, _r, "Value");
            m_kGradeById[rec.uiID] = m_kGrade.size();
            m_kGrade.push_back(rec);
        }
    }
